...
const UsersReport: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const resp: any = await api.getReports ? await api.getReports('user') : { reports: [] };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      {reports.length === 0 ? (
        <div className="text-gray-600">No user reports (backend endpoint /reports not implemented)</div>
      ) : (
        <ul className="space-y-3">
          {reports.map(r => (
            <li key={r.id} className="p-3 bg-white rounded shadow">
              <div>User ID: {r.target_id}</div>
